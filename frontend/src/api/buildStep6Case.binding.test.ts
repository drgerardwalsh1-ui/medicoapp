...
describe("buildStep6Case IPC binding", () => {
  // 1. frontend_can_invoke_build_step6_case
  it("frontend_can_invoke_build_step6_case", async () => {
    ...
    await TauriAPI.buildStep6Case(clinicalEvents, [], [], participants);

    expect(invokeMock).toHaveBeenCalledTimes(1);
    const [cmd, args] = invokeMock.mock.calls[0];
    // Correct backend command name.
    expect(cmd).toBe("build_step6_case");
