/**
 * CONTRADICTION OBSERVABILITY — a collapsible analysis block inside the Demographics
 * Documents stack.
 *
 ...
 *   - ALL data comes from ONE backend object: `ContradictionObservabilityRoot`. The UI
...
import {
  ContradictionObservabilityCtx,
  useContradictionObservability,
} from "./contradictionObservabilityContext";
import {
  runContradictionObservabilityForClient,
  type ContradictionObservabilityRoot,
  type ContradictionPriority,
  type ContradictionReadinessLevel,
  type ContradictionTrendDirection,
} from "../lib/contradictionEngine";

// ── Provider (shared lazy fetch + cache) ─────────────────────────────────────

export function ContradictionObservabilityProvider({
  clientId,
  children,
}: {
  ...
  const rootRef = useRef<ContradictionObservabilityRoot | null>(null);
  const inflightRef = useRef<Promise<ContradictionObservabilityRoot | null> | null>(null);
  const [root, setRoot] = useState<ContradictionObservabilityRoot | null>(null);
  ...
  const ensure = useCallback(async (): Promise<ContradictionObservabilityRoot | null> => {
    ...
    setLoading(true);
    setError(null);
    const p = runContradictionObservabilityForClient(clientId)
      .then((r) => {
        rootRef.current = r;
        setRoot(r);
        setLoading(false);
  ...
  return (
    <ContradictionObservabilityCtx.Provider value={{ root, loading, error, available, ensure }}>
      {children}
    </ContradictionObservabilityCtx.Provider>
  );
}

// ── Section ──────────────────────────────────────────────────────────────────

export default function ContradictionObservabilitySection() {
  const { available } = useContradictionObservability();
  if (!available) return null;

  return (
    <CollapsibleSection
      docId="case"
      sectionId="contradiction-observability"
      kind="contradiction"
      title="CONTRADICTION OBSERVABILITY"
      defaultOpen={false}
    ...
    >
      <ContradictionObservabilityBody />
    </CollapsibleSection>
  );
}

/** Mounted only while the section is open (lazy) — triggers the single fetch. */
function ContradictionObservabilityBody() {
  const { root, loading, error, ensure } = useContradictionObservability();
  ...
  if (error) {
    return (
      <div className="text-xs text-rose-600">
        Failed to load contradiction observability: {error}
      </div>
    );
  }
  if (!root) {
    return (
      <div className="text-xs text-slate-400">
        {loading ? "Loading contradiction observability…" : "No data."}
      </div>
    );
  }

  return <ContradictionObservabilityContent root={root} />;
}
...
function ContradictionObservabilityContent({ root }: { root: ContradictionObservabilityRoot }) {
  const r = root.snapshot.report;
  const gs = r.analytics.graph_summary;

  const trendIds = (d: ContradictionTrendDirection) =>
    root.trends.trends.filter((t) => t.direction === d).map((t) => t.contradiction_id);
  const readyIds = (lvl: ContradictionReadinessLevel) =>
    root.readiness.items.filter((i) => i.readiness === lvl).map((i) => i.contradiction_id);
  const queueIds = (p: ContradictionPriority) =>
