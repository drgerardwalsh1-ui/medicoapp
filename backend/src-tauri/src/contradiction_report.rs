//! Canonical Contradiction Engine report — the single, stable aggregation object that future
...
use crate::contradiction_cluster_graph::{build_cluster_graph, ClusterGraph};
use crate::cluster_graph_analysis::{analyze_graph, ClinicalSignificance, GraphAnalytics};
use crate::cluster_graph_narrative::{build_graph_narrative, GraphNarrative};
...
/// Assemble the canonical report from a `ContradictionView`: build the graph, analyse
/// it, narrate it, and bundle. No recomputation beyond the existing builders;
/// the input `view` is carried verbatim into the report.
pub fn build_contradiction_report(view: ContradictionView) -> ContradictionReport {
    let graph = build_cluster_graph(&view);
    let analytics = analyze_graph(&graph);
    let narrative = build_graph_narrative(&graph, &analytics);
    ContradictionReport { view, graph, analytics, narrative }
}

// ── Pure queries ────────────────────────────────────────────────────────────

pub fn contradiction_count(report: &ContradictionReport) -> usize {
    report.view.enriched_contradictions.len()
}

pub fn cluster_count(report: &ContradictionReport) -> usize {
    report.analytics.cluster_metrics.len()
}

/// Clusters whose significance is Medium OR High.
pub fn significant_cluster_count(report: &ContradictionReport) -> usize {
    report
        .analytics
        .cluster_metrics
        .iter()
...
pub fn validate_report(report: &ContradictionReport) -> ReportIntegrity {
    ReportIntegrity {
...
mod tests {
    ...
    fn report_build_is_deterministic() {
        let r1 = build_contradiction_report(sample_view());
        let r2 = build_contradiction_report(sample_view());
        assert_eq!(
    ...
    fn report_contains_all_layers() {
        let r = build_contradiction_report(sample_view());
    ...
    fn validate_report_detects_mismatch() {
        let mut r = build_contradiction_report(sample_view());
        ...
        let mut r2 = build_contradiction_report(sample_view());
        r2.graph.nodes.pop();
    ...
    fn report_builder_does_not_modify_view() {
        ...
        let r = build_contradiction_report(view);
