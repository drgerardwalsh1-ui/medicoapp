//! Production input adapter for the STEP-6 pipeline (closes audit F-1 + F-2).
//!
...
//! into `Vec<EventEnvelope>`, then invokes the existing `build_step6_pipeline`.
//!
...
use crate::step6_export::Step6Export;
use crate::step6_pipeline::{build_step6_pipeline, Step6PipelineOutput};
...
/// Production entrypoint (orchestration delegate): adapt → existing pipeline.
/// No recomputation, no duplicate orchestration — it simply calls
/// `build_step6_pipeline`.
pub fn run_step6_from_extraction(
    clinical_events_json: &[serde_json::Value],
    family_events: Vec<FamilyEvent>,
    legal_events: Vec<LegalStatusEvent>,
    participants: &[Participant],
) -> Step6PipelineOutput {
    let envelopes = extraction_to_event_envelopes(clinical_events_json, family_events, legal_events);
    build_step6_pipeline(envelopes, participants)
}

/// Production entrypoint WITH fact extraction. Identical to
/// `run_step6_from_extraction`, plus: each `(doc_id, clean_text)` blob is run
/// through `fact_extract::extract_facts` and the resulting facts are appended as
/// `EventEnvelope::Fact` so value-disagreement contradictions appear in STEP-6.
/// Facts from ALL documents are pooled before contradiction generation, so a
/// disagreement spanning two documents (e.g. injury date 12 Feb in doc A vs
/// 18 Feb in doc B) is detected. Strictly additive — the clinical/family/legal
/// path is unchanged.
pub fn run_step6_from_extraction_with_text(
    clinical_events_json: &[serde_json::Value],
...
/// materialised outputs as `Step6PipelineOutput`, verbatim.
#[derive(Serialize)]
pub struct Step6Result {
    pub case: CanonicalCase,
    pub view: Step6View,
    pub export: Step6Export,
    pub csv_lines: Vec<String>,
}

impl From<Step6PipelineOutput> for Step6Result {
    fn from(o: Step6PipelineOutput) -> Self {
        Step6Result {
            case: o.case,
            view: o.view,
            export: o.export,
...
mod tests {
    ...
    fn production_entrypoint_invokes_existing_pipeline_only() {
        ...
        let via_entrypoint = run_step6_from_extraction(&clinical, fam.clone(), leg.clone(), &[]);
        ...
        let direct = build_step6_pipeline(envelopes, &[]);
    ...
    fn legal_events_reach_case_timeline_via_real_adapter() {
        let out = run_step6_from_extraction(&[], vec![], vec![legal("l1", LegalRole::Applicant)], &[]);
    ...
    fn family_events_reach_family_contradictions_via_real_adapter() {
        let out = run_step6_from_extraction(
            &[],
    ...
    fn injury_events_reach_clinical_contradictions_via_real_adapter() {
        let out = run_step6_from_extraction(&mixed_clinical(), vec![], vec![], &[]);
        let clin: Vec<_> = out
            .case
            .contradictions
            .iter()
    ...
    fn pipeline_output_is_identical_to_manual_build() {
        ...
        let adapted = run_step6_from_extraction(&clinical, fam.clone(), leg.clone(), &[]);
        // Path 2: manual envelopes (the adapter's own output) → pipeline.
        let manual = build_step6_pipeline(
    ...
    fn input_order_independence() {
        ...
        let a = run_step6_from_extraction(&forward, vec![], vec![], &[]);
        let b = run_step6_from_extraction(&reversed, vec![], vec![], &[]);
    ...
    fn symptom_corpus_produces_namespaced_clinical_contradiction() {
        ...
        let out = run_step6_from_extraction(&events, vec![], vec![], &[]);
        assert_eq!(out.case.contradictions.len(), 1);
        let c = &out.case.contradictions[0];
        assert_eq!(c.domain, ContradictionDomain::Clinical);
