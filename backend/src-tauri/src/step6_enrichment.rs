//! STEP-6 enrichment — a PURE, POST-PROCESSING VIEW LAYER.
//!
//! Operates ONLY on the already-built `Vec<CaseContradiction>` produced by the
//! STEP-6 assembler. It never touches the clinical/family/legal engines, never
...
pub struct EnrichedCaseContradiction {
    /// Verbatim, unmodified STEP-6 contradiction.
    pub base: CaseContradiction,
    pub derived: Step6DerivedFields,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Step6View {
...
/// Enrich the STEP-6 contradiction stream into a view. Input is consumed and
/// returned verbatim inside each `base`; nothing is mutated or recomputed.
pub fn enrich_step6(input: Vec<CaseContradiction>) -> Step6View {
    // First pass: derive purely from the (immutable) input slice.
    let derived: Vec<Step6DerivedFields> = (0..input.len())
        .map(|i| Step6DerivedFields {
    ...
    Step6View { enriched_contradictions, cross_namespace_relations }
}
...
mod tests {
    ...
    fn presentation_group_is_domain_colon_label() {
        let v = enrich_step6(vec![cc(ContradictionDomain::Clinical, "assertion", "ptsd", 0.5, true)]);
    ...
    fn enrichment_is_deterministic() {
        ...
        let a = enrich_step6(mk());
        let b = enrich_step6(mk());
        assert_eq!(
    ...
    fn base_is_unmodified() {
        ...
        let view = enrich_step6(vec![original]);
    ...
    fn export_rows_remain_byte_identical() {
        use crate::step6_export::export_rows;
        let v = diag_sym_view();
    ...
    fn cross_namespace_does_not_mutate_base() {
        ...
        let snapshot = serde_json::to_string(&original).unwrap();
        let v = enrich_step6(vec![original]);
