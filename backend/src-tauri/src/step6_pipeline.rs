//! STEP-6 pipeline — the single end-to-end integration seam (audit F1 + F2).
//!
...
//!     → step6_enrichment::enrich_step6
//!     → step6_export::{export_rows, export_csv_lines}
//!
...
use crate::step6_enrichment::{enrich_step6, Step6View};
use crate::step6_export::{export_csv_lines, export_rows, Step6Export};

/// The full materialised STEP-6 result from a single event batch.
pub struct Step6PipelineOutput {
...
/// Route a batch of events through the entire STEP-6 stack and materialise the
/// case, enriched view, and export projections. Pure orchestration.
pub fn build_step6_pipeline(
    ...
    // intact (the enriched view is the integration surface for STEP-6).
    let fact_contradictions = build_fact_contradictions(fact_assertions);

    // 7–9. Enrich + project. `enrich_step6` consumes by value, so clone the
    ...
    let view = enrich_step6(contradiction_stream);
    let export = export_rows(&view);
    let csv_lines = export_csv_lines(&view);

    Step6PipelineOutput { case, view, export, csv_lines }
}
...
mod tests {
    ...
    /// dispatch → build_fact_contradictions → enrich_step6 → graph nodes.
    #[test]
    fn fact_value_contradictions_reach_graph_nodes() {
        use crate::fact_extract::extract_facts;
        use crate::step6_graph::build_step6_graph;
        ...
        let out = build_step6_pipeline(envelopes, &[]);
        ...
        let graph = build_step6_graph(&out.view);
    ...
    fn facts_and_clinical_contradictions_coexist() {
        ...
        let graph = crate::step6_graph::build_step6_graph(&out.view);
        let diag_node = graph
            .nodes
            .iter()
    ...
    fn dispatch_to_export_end_to_end_is_byte_identical() {
        let o1 = build_step6_pipeline(mixed_batch(), &[]);
        let o2 = build_step6_pipeline(mixed_batch(), &[]);
    ...
    fn legal_events_reach_case_timeline() {
        let out = build_step6_pipeline(vec![legal_env("l1", LegalRole::Applicant)], &[]);
    ...
    fn legal_events_do_not_affect_contradictions() {
        let without = build_step6_pipeline(
            vec![
                injury_env("i1", "fractured wrist", AssertionStatus::Affirmed, "docA"),
                injury_env("i2", "fractured wrist", AssertionStatus::Negated, "docB"),
                family_env("f1", FamilyRelation::SiblingOf, "docC"),
        ...
        let with = build_step6_pipeline(mixed_batch(), &[]);
    ...
    fn pipeline_preserves_domain_separation() {
        let out = build_step6_pipeline(mixed_batch(), &[]);
    ...
    #[test]
    fn step6_end_to_end_pipeline_with_provenance() {
        ...
        let csv2 = build_step6_pipeline(mixed_batch(), &[]).csv_lines;
