//! STEP-6 observability root — a COMPOSITION-ONLY layer that bundles every
//! derived STEP-6 view (history, trends, readiness, queue, dashboard) for a
...
use crate::step6_forecast_readiness::{build_forecast_readiness, ForecastReadinessReport};
use crate::step6_monitoring_dashboard::{build_monitoring_dashboard, MonitoringDashboard};
use crate::step6_monitoring_queue::{build_monitoring_queue, MonitoringQueue};
use crate::step6_report::{build_step6_report, cluster_count, contradiction_count};
use crate::step6_report_history::{build_report_history, ReportHistory};
use crate::step6_report_snapshot::Step6ReportSnapshot;
use crate::step6_report_trends::{build_report_trends, ReportTrends};

// ── Model ─────────────────────────────────────────────────────────────────────

#[derive(Debug, Clone, Serialize)]
pub struct Step6ObservabilityRoot {
    pub snapshot: Step6ReportSnapshot,
...
pub fn build_observability_root(snapshot: Step6ReportSnapshot) -> Step6ObservabilityRoot {
...
pub fn observability_root_is_consistent(root: &Step6ObservabilityRoot) -> bool {
    // 1. snapshot.report reconstructs from its own view (no recomputation drift).
    let rebuilt = build_step6_report(root.snapshot.report.view.clone());
    let report_ok =
...
pub fn all_contradiction_ids(root: &Step6ObservabilityRoot) -> Vec<String> {
...
pub fn observability_summary(root: &Step6ObservabilityRoot) -> String {
    format!(
...
mod tests {
    ...
    fn snapshot() -> Step6ReportSnapshot {
        let ccs = vec![
            cc("i1", "a", 0.5),
            cc("i2", "b", 0.5),
            cc("i3", "c", 0.5),
        ];
        build_report_snapshot(build_step6_report(enrich_step6(ccs)), 1_000)
    }
