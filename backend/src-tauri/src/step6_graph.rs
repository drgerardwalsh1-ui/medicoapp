//! STEP-6 semantic graph — a PURE DERIVATION LAYER above `Step6View`.
//!
...
//! verbatim from `Step6View.cross_namespace_relations`. Supports traversal,
//! clustering (connected components), and filtered queries.
//!
//! It NEVER touches the engine, enrichment, or export: `build_step6_graph`
//! takes `&Step6View` by reference and reads only. Determinism: every output
...
use crate::step6_enrichment::{CrossNamespaceRelation, Step6View};
...
/// Build the graph from a `Step6View`. Nodes map 1:1 to enriched
/// contradictions; edges come only from `cross_namespace_relations`.
pub fn build_step6_graph(view: &Step6View) -> Step6Graph {
...
pub fn traverse_graph(graph: &Step6Graph, start_id: &str, max_depth: usize) -> Vec<String> {
...
pub fn compute_clusters(graph: &Step6Graph) -> Vec<Vec<String>> {
    let mut visited: BTreeSet<String> = BTreeSet::new();
...
pub fn query_graph(graph: &Step6Graph, query: &GraphQuery) -> Step6Graph {
...
mod tests {
    ...
    fn graph_is_deterministic_across_runs() {
        let v = sample_view();
        let g1 = build_step6_graph(&v);
        let g2 = build_step6_graph(&v);
        assert_eq!(g1, g2);
        assert_eq!(
    ...
    fn adjacency_lists_are_sorted() {
        let g = build_step6_graph(&sample_view());
        // Keys sorted.
