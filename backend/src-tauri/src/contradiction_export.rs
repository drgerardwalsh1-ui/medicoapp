//! Contradiction Engine export — deterministic, read-only PROJECTIONS of the enriched stream.
//!
//! Consumes `contradiction_enrichment::ContradictionView` ONLY. It never touches a graph, an
...
pub struct ContradictionExportRow {
    pub domain: String,
    pub subject: String,
...
#[derive(Debug, Clone, PartialEq, serde::Serialize)]
pub struct ContradictionExport {
    pub rows: Vec<ContradictionExportRow>,
}
...
pub fn export_rows(view: &ContradictionView) -> ContradictionExport {
    let rows = view
        .enriched_contradictions
        .iter()
        .map(|e| {
            let (source_ids, edge_ids) = provenance_of(&e.base);
            ContradictionExportRow {
...
fn row_to_csv(r: &ContradictionExportRow) -> String {
    format!(
        "{},{},{},{:.*},{},{},{},{},{},{}",
        csv_escape(&r.domain),
...
pub fn export_csv_lines(view: &ContradictionView) -> Vec<String> {
...
/// key-sorted ordering.
pub fn export_summary(view: &ContradictionView) -> BTreeMap<String, usize> {
...
mod tests {
    ...
    fn csv_escapes_commas_deterministically() {
        let v = enrich_contradictions(vec![cc(ContradictionDomain::Family, "relational", "a, b", 0.5)]);
    ...
    fn export_rows_preserve_family_edge_ids() {
        ...
        let v = enrich_contradictions(vec![family_cc_prov("a↔b", 0.5, &["docB", "docA"], &["e2", "e1"])]);
    ...
    fn export_rows_preserve_clinical_source_ids() {
        let v = enrich_contradictions(vec![clinical_cc_prov("ptsd", 0.5, &["u2", "u1"], &["p1"])]);
        let row = &export_rows(&v).rows[0];
    ...
    fn export_csv_confidence_formatting_is_pinned() {
        ...
        let v = enrich_contradictions(vec![family_cc_prov("x", rc, &[], &[])]);
        ...
        let v2 = enrich_contradictions(vec![family_cc_prov("y", 0.5, &[], &[])]);
    ...
    fn export_csv_confidence_roundtrip_stable() {
        let v = enrich_contradictions(vec![family_cc_prov("x", 0.667, &[], &[])]);
        let a = export_csv_lines(&v);
        let b = export_csv_lines(&v);
        assert_eq!(a, b);
