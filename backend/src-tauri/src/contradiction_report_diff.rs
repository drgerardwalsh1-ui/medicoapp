//! Contradiction Engine snapshot diff — deterministic, read-only comparison of two
//! `ContradictionReportSnapshot`s. Answers "what changed between two reports?" using
...
use crate::contradiction_snapshot_repository::{newest_snapshot, SnapshotRepository};
...
fn contradiction_ids(snapshot: &ContradictionReportSnapshot) -> BTreeSet<String> {
    snapshot
        .report
        .view
...
fn cluster_len(s: &ContradictionReportSnapshot) -> usize {
    s.report.analytics.cluster_metrics.len()
}
fn contradiction_len(s: &ContradictionReportSnapshot) -> usize {
    s.report.view.enriched_contradictions.len()
}
fn node_len(s: &ContradictionReportSnapshot) -> usize {
    s.report.graph.nodes.len()
}
fn edge_len(s: &ContradictionReportSnapshot) -> usize {
...
mod tests {
    ...
    fn snap(subjects: &[&str], ts: u64) -> ContradictionReportSnapshot {
