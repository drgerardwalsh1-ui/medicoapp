//! STEP-6 longitudinal history — deterministic, read-only analysis across MANY
...
pub fn build_report_history(snapshots: &[Step6ReportSnapshot]) -> ReportHistory {
...
pub fn build_history_from_repository(repo: &dyn SnapshotRepository) -> ReportHistory {
    let snapshots: Vec<Step6ReportSnapshot> =
...
mod tests {
    ...
    fn growing() -> Vec<Step6ReportSnapshot> {
        vec![
            snap(&[("i1", "a", 0.5)], 1),
