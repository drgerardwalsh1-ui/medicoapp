//! Durable STEP-6 report snapshot — a versioned, serializable artifact wrapping
//! the canonical `Step6Report` so persistence, APIs, caching, PDF generation,
//! and audit storage all consume the same object.
//!
//! PURE WRAPPING ONLY: it adds a schema version, a deterministic id, and a
//! caller-supplied timestamp around an existing `Step6Report`. No engine, graph,
...
use crate::step6_report::{cluster_count, contradiction_count, Step6Report};

/// Bumped only on a breaking change to the snapshot/report schema.
pub const STEP6_REPORT_SCHEMA_VERSION: &str = "1.0";

// ── Snapshot model ──────────────────────────────────────────────────────────

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Step6ReportSnapshot {
    pub schema_version: String,
    pub report_id: String,
    /// Caller-supplied creation time (epoch milliseconds). Never generated here.
    pub created_at_epoch_ms: u64,
    pub report: Step6Report,
}

// ── Builder ─────────────────────────────────────────────────────────────────

/// Wrap a `Step6Report` in a versioned snapshot. `report_id` is a stable,
...
pub fn build_report_snapshot(
    ...
    Step6ReportSnapshot {
        schema_version: STEP6_REPORT_SCHEMA_VERSION.to_string(),
        report_id,
        created_at_epoch_ms,
        report,
    }
...
pub fn snapshot_summary(snapshot: &Step6ReportSnapshot) -> String {
    format!(
...
pub fn snapshot_is_current(snapshot: &Step6ReportSnapshot) -> bool {
    snapshot.schema_version == STEP6_REPORT_SCHEMA_VERSION
}
...
pub fn check_snapshot_compatibility(snapshot: &Step6ReportSnapshot) -> SnapshotCompatibility {
...
mod tests {
    ...
    fn schema_version_is_set() {
        ...
        assert_eq!(s.schema_version, STEP6_REPORT_SCHEMA_VERSION);
    }
