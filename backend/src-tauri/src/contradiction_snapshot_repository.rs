//! Contradiction Engine snapshot persistence boundary — the repository CONTRACT that future
...
pub trait SnapshotRepository {
    ...
    fn save(&mut self, snapshot: ContradictionReportSnapshot) -> Result<(), String>;

    /// Fetch a snapshot by its `report_id`, if present.
    fn get(&self, report_id: &str) -> Option<ContradictionReportSnapshot>;
...
pub struct InMemorySnapshotRepository {
    store: BTreeMap<String, ContradictionReportSnapshot>,
}
...
pub fn newest_snapshot(repo: &dyn SnapshotRepository) -> Option<ContradictionReportSnapshot> {
    repo.list()
        .into_iter()
...
mod tests {
    ...
    fn snap(n: usize, ts: u64) -> ContradictionReportSnapshot {
        let mut ccs = Vec::new();
        for i in 0..n {
            ccs.push(cc(
    ...
    fn repository_consistency_detects_manual_corruption() {
        ...
        impl SnapshotRepository for Corrupt {
            fn save(&mut self, _s: ContradictionReportSnapshot) -> Result<(), String> {
                Ok(())
            }
            fn get(&self, _id: &str) -> Option<ContradictionReportSnapshot> {
